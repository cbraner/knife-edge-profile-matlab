function blade_workflow_v3()
% =========================================================
% blade_workflow_v3.m
% זרימה:
% 1) בחירת NEF → הפקת GREEN מלאה (ממוצע G1,G2)
% 2) יישור פרספקטיבה על כל הפריים (בחירת 4 נקודות על GREEN)
% 3) הצגת GREEN המיושר ואז דיאלוג סיבוב (אם צריך)
% 4) בחירת מצב תצוגה (Gamma/CLAHE) → הצגה של מצב יחיד במסך מלא למעלה
%    עם גאמה 0.5 ו-stretchlim להגברת בהירות/קונטרסט → בחירת 2 קצות להב
% 5) פרופיל לאורך 61 מ״מ + שמירה כ-FIG ו-PNG
% =========================================================
...
green_warp = fliplr(rot90(imwarp(green, tform, 'OutputView', RoutG),-1));
...
switch ansRot
    case 'ימינה 90°',  green_warp = fliplr(imrotate(green_warp, -90));
    case 'שמאלה 90°',  green_warp = imrotate(green_warp,  90);

end

%% (4) בחירת מצב תצוגה יחיד (Gamma או CLAHE) + בחירת 2 קצות להב
% בסיס לתצוגה: מתיחה לפי אחוזונים ואז גאמה/CLAHE
base = mat2gray(green_warp);
lims = stretchlim(base, [0.01 0.99]);  % הגברת בהירות/קונטרסט עדינה

modeSel = questdlg('באיזה מצב תרצה לבחור קצות להב?', 'בחירת מצב תצוגה', ...
                   'Gamma (בהיר)','CLAHE (מקומי)','Gamma (בהיר)');

switch modeSel
    case 'Gamma (בהיר)'
        gamma = 0.5;                          % לפי בקשתך
        disp_img = imadjust(base, lims, [], gamma);
        titleStr = sprintf('GREEN (Gamma=%.2f + stretchlim)', gamma);
    otherwise
        disp_img = adapthisteq(imadjust(base, lims, []), 'NumTiles',[8 8], 'ClipLimit',0.01);
        titleStr = 'GREEN (CLAHE + stretchlim)';
end

% פותחים חלון חדש יחיד, במסך מלא ולמעלה, רק עם מצב שנבחר
figPick = fullscreenFigure('בחר 2 קצות להב (מסך מלא – מצב יחיד)');
figure(figPick);  % מביא לקדמה
imshow(disp_img, 'InitialMagnification','fit'); axis on; impixelinfo
title({titleStr, 'הקלק 2 נקודות קצה להב, סיים ב-Enter/דאבל-קליק'}, 'FontSize', 12);

[xb,yb] = getpts;  assert(numel(xb)>=2,'נדרשות לפחות 2 נקודות');
bp1 = [round(xb(1)), round(yb(1))];
bp2 = [round(xb(2)), round(yb(2))];

%% (5) פרופיל לאורך 61 מ״מ + שמירה
% פרמטרי חלון דגימה
...
% שמירות
savefig(figProf, fullfile(outDir, sprintf('profile_%s_%s.fig', nefBase, ts)));
exportgraphics(figProf, fullfile(outDir, sprintf('profile_%s_%s.png', nefBase, ts)));

disp(['נשמרו קבצים בתיקייה "', outDir, '"']);

end % == blade_workflow_v3 ==
...
function f = fullscreenFigure(nameStr)
    f = figure('Name',nameStr,'NumberTitle','off','WindowStyle','docked');
    try, set(f,'WindowState','maximized'); catch
        set(f,'Units','normalized','OuterPosition',[0 0 1 1]);
    end
    drawnow;  % דואג שהחלון יופיע ויעלה קדימה
end
